module suidex::pool {
    ...
    public fun get_amount_out(
        ...
        // Standard Uniswap formula: amount_out = (amount_in * (10000 - fee_bps) * reserve_out) / (reserve_in * 10000 + amount_in * (10000 - fee_bps))
        let amount_in_with_fee = safe_mul(amount_in, (FEE_DENOMINATOR - fee_bps));
        
        // Check if we need to use scaled calculation for large numbers
        let is_large_number = amount_in_with_fee > (U64_MAX / reserve_out) || 
                             reserve_in > (U64_MAX / FEE_DENOMINATOR) ||
                             amount_in_with_fee > (U64_MAX / 100);
        
        if (is_large_number) {
            // Use scaling to prevent overflow
            let scaled_amount_in_with_fee = safe_div(amount_in_with_fee, SCALE_FACTOR);
            ...
            let safe_scaled_reserve_out = if (scaled_reserve_out == 0) { 1 } else { scaled_reserve_out };
            let safe_scaled_amount_in_with_fee = if (scaled_amount_in_with_fee == 0) { 1 } else { scaled_amount_in_with_fee };
            
            // Calculate scaled numerator and denominator
            let scaled_numerator = safe_mul(safe_scaled_amount_in_with_fee, safe_scaled_reserve_out);
            let scaled_denominator = safe_mul(safe_scaled_reserve_in, FEE_DENOMINATOR) + safe_scaled_amount_in_with_fee;
            ...
            let denominator = safe_mul(reserve_in, FEE_DENOMINATOR) + amount_in_with_fee;
            
            // Ensure denominator is not zero
            let safe_denominator = if (denominator == 0) { 1 } else { denominator };
            
            safe_div(numerator, safe_denominator)
        }
    }
